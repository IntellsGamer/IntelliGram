...
import type TcpAbridged from '@lib/mtproto/transports/tcpAbridged';
import sessionStorage from '@lib/sessionStorage';
import MTPNetworker, {MTMessage} from '@lib/mtproto/networker';
import {ConnectionType, constructIntelliGramWebSocketUrl, DcConfigurator, TransportType} from '@lib/mtproto/dcConfigurator';
...
export class ApiManager extends ApiManagerMethods {
  ...
  protected after() {
    ...
    // All traffic, including file transfers, remains on the same self-hosted
    // IntelliGram endpoint; premium state must not alter routing.
    this.rootScope.addEventListener('premium_toggle', () => {
      this.iterateNetworkers(({networker, connectionType, dcId}) => {
        const transport = networker.transport as TcpAbridged | undefined;
        transport?.changeUrl(constructIntelliGramWebSocketUrl(dcId, connectionType));
      });
    });
