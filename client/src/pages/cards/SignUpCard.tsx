...
export default function SignUpCard(props: {spec: Spec}) {
  ...
  (passwordInputField.input as HTMLInputElement).type = 'password';
  ...
  function onSubmit() {
    ...
    const params = {
      ...
      // The server recognizes this self-hosted extension only after the
      // SMS-free registration route selected `intelligram-register`.
      phone_code_hash: `${props.spec.payload.phone_code_hash}:${btoa(unescape(encodeURIComponent(passwordInputField.value)))}`,
      first_name: name,
      last_name: lastName
    };
